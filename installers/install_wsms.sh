...
# SMTP Email Alert Settings (Single Source of Truth)
SMTP_ENABLED="no"                      # "yes" to auto-configure ~/.msmtprc, "no" to disable
ALERT_EMAIL="from_you@email.com"       # destination email for alerts
ALERT_ON_FAILURE="yes"                 # send alert on critical failure
ALERT_ON_SUCCESS="no"                  # send alert on successful cron run
SMTP_HOST="SMTP_SERVER"                # e.g. ssl0.ovh.net or smtp.gmail.com
SMTP_PORT="587"                        # 587 (STARTTLS) or 465 (SSL)
SMTP_USER="YOUR_LOGIN(user@email.com)" # your smtp username / email
SMTP_PASS="YOUR_PASSWORD"              # your smtp password
SMTP_FROM="from_you@email.com"         # sender email address
SMTP_TLS="on"
SMTP_STARTTLS="on"
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.3 - CENTRAL CONFIGURATION
...
# SMTP Relay Configuration (auto-generates ~/.msmtprc)
SMTP_ENABLED="no"
SMTP_HOST=""
SMTP_PORT="587"
SMTP_USER=""
...
ensure_msmtprc() {
    ...
    local new_content
    new_content="# Auto-generated by WSMS PRO from wsms-config.sh (SSOT)
defaults
auth           on
tls            ${SMTP_TLS:-on}
...
send_alert() {
    ...
    if command -v mail >/dev/null 2>&1; then
        ...
        echo "WSMS alert error: neither 'mail' nor 'msmtp' command is available." >&2
        return 1
    fi
}
EOFNOTIFY
...
for site in "${SITES[@]}"; do
    IFS=':' read -r name path user <<< "$site"
    printf "  ${GREEN}%-46s${NC} %s\n" "wp-backup-site $name" "Site backup for $name"
...
if [ -n "$target" ]; then
    ...
    if [ "$found" = true ]; then
        bash "$SCRIPT_DIR/wp-essential-assets-backup.sh" "$target"
        exit $?
    else
        echo "❌ Site not found: $target"
        echo "Available sites:"
        for site in "${SITES[@]}"; do
            IFS=':' read -r name path user <<< "$site"
            echo " - $name"
...
alias mysql-backup-site='db-backup'
...
# ==================== FINAL SUMMARY ====================
echo -e "\n${GREEN}==========================================================${NC}"
echo -e "${GREEN}✅ WSMS PRO v4.4.3 INSTALLATION COMPLETE!${NC}"
echo -e "${GREEN}==========================================================${NC}"
echo ""
