#!/bin/bash
# =================================================================
# 🚀 WSMS PRO v4.4.2 - UNIVERSAL INSTALLER
# Version: 4.4.2 | Works in any shell (Bash, Fish, Zsh, Sh)
...
echo -e "${CYAN}==========================================================${NC}"
echo -e "${CYAN}   🚀 WSMS PRO v4.4.2 - UNIVERSAL INSTALLER                  ${NC}"
...
# Organized system log directories (/var/log/wsms/)
LOG_DIRS=(
...
if sudo mkdir -p /var/quarantine /var/log/clamav /var/log/wsms "${LOG_DIRS[@]}"; then
    log_success "Created system directories (/var/quarantine, /var/log/clamav, /var/log/wsms)"
else
...
if sudo chown root:sudo /var/quarantine 2>/dev/null && sudo chmod 750 /var/quarantine 2>/dev/null; then
    log_success "Permissions set for /var/quarantine (root:sudo 750)"
...
# Permissions for /var/log/wsms: owner $USER, group sudo + SGID flag (2775)
if sudo chown -R "$USER:sudo" /var/log/wsms 2>/dev/null && sudo chmod -R 2775 /var/log/wsms 2>/dev/null; then
    log_success "Permissions set for /var/log/wsms ($USER:sudo 2775 SGID)"
elif sudo chown -R root:sudo /var/log/wsms 2>/dev/null && sudo chmod -R 2775 /var/log/wsms 2>/dev/null; then
    log_success "Permissions set for /var/log/wsms (root:sudo 2775 SGID)"
elif sudo chmod -R 775 /var/log/wsms 2>/dev/null; then
    log_warning "Permissions set to 775 for /var/log/wsms"
fi
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - CENTRAL CONFIGURATION
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - EMAIL ALERT MODULE
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - DAILY SYSTEM CHECK
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - ENHANCED EXECUTIVE DIAGNOSTICS
# =================================================================
...
clear
echo -e "${BLUE}🖥️  WSMS EXECUTIVE DIAGNOSTICS v4.4.2${NC}"
echo "=========================================================="
...
if [ -n "$disk_usage" ] && [ "$disk_usage" -ge "$DISK_ALERT_THRESHOLD" ]; then
    echo -e "   ⚠️  ${RED}CRITICAL: Disk usage at ${disk_usage}% - run backup-clean!${NC}"
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - WORDPRESS FLEET STATUS MONITOR
# =================================================================

source "$HOME/scripts/wsms-config.sh"
GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'; RED='\033[0;31m'; NC='\033[0m'

echo -e "${CYAN}📊 WORDPRESS FLEET STATUS v4.4.2${NC}"
echo "=========================================================="
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - MULTI-INSTANCE DEEP AUDIT
# =================================================================

source "$HOME/scripts/wsms-config.sh"
CYAN='\033[0;36m'; YELLOW='\033[1;33m'; GREEN='\033[0;32m'; RED='\033[0;31m'; NC='\033[0m'

echo -e "${CYAN}🔍 INITIATING MULTI-SITE DEEP AUDIT v4.4.2${NC}"
echo "=========================================================="
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - FLEET-WIDE MAINTENANCE ENGINE
# =================================================================
...
echo "=========================================================="
echo "🔄 MAINTENANCE ENGINE v4.4.2 - $(date)"
echo "=========================================================="
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - INFRASTRUCTURE PERMISSION ORCHESTRATOR
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - FULL RECOVERY BACKUP
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - ESSENTIAL ASSETS BACKUP (LITE)
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - MYSQL BACKUP MANAGER
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - NAS SFTP SYNC (Production)
# =================================================================
...
ensure_remote_dir() {
    ...
    if echo "ls \"$remote_dir\"" 2>/dev/null | sftp -i "$SSH_KEY" -P "$REMOTE_PORT" -o StrictHostKeyChecking=no "$REMOTE_USER@$REMOTE_SERVER" 2>/dev/null | grep -qF "$remote_dir"; then
        return 0
    fi
...
remote_file_exists() {
    ...
    get_remote_files_list "$remote_dir" | grep -Fxq "$file_name"
}
...
sync_directory() {
    ...
    done < <(find "$local_dir" -maxdepth 1 -type f -exec basename {} \; 2>/dev/null)
    ...
    done < <(echo "$remote_files_list" | sort -r)
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - SMART RETENTION MANAGER
# =================================================================
...
list_backups() {
    echo -e "${CYAN}📋 ALL BACKUPS WITH DETAILS v4.4.2${NC}"
    echo "=========================================================="
...
show_size() {
    echo -e "${CYAN}💽 BACKUP STORAGE USAGE v4.4.2${NC}"
    echo "=========================================================="
    ...
    if [ "$disk_usage" -ge "$DISK_ALERT_THRESHOLD" ]; then
        ...
        echo -e "   ${YELLOW}💡 Run 'backup-clean' to free space urgently (choose emergency mode in menu)${NC}"
    fi
}
...
emergency_purge_all() {
    echo -e "${RED}☢️ EMERGENCY PURGE: Delete ALL backups and rollback snapshots${NC}"
    echo "=========================================================="
    echo -e "${YELLOW}This will permanently remove all files from:${NC}"
    ...
    for dir in "$BACKUP_LITE_DIR" "$BACKUP_FULL_DIR" "$BACKUP_MYSQL_DIR"; do
        ...
        echo "   🗑️ Cleared files in $(basename "$dir")"
    done
    ...
    if [ -d "$BACKUP_ROLLBACK_DIR" ]; then
        ...
        echo "   🗑️ Cleared snapshots in $(basename "$BACKUP_ROLLBACK_DIR")"
    fi

    echo -e "\n${GREEN}✅ EMERGENCY PURGE COMPLETE — removed entries: $deleted${NC}"
}
...
interactive_clean() {
    ...
    echo "   7) EMERGENCY DEEP CLEAN (keep 1 newest backup per site/group + 1 snapshot per site)"
    echo "   8) HOUSEKEEPING (old logs + old bashrc/crontab backups)"
    echo "   9) EMERGENCY PURGE ALL (delete all backups and snapshots)"
    echo "   0) Cancel"
    echo ""
    ...
    case $choice in
        ...
        5) force_clean ;;
        6) emergency_cleanup ;;
        7) emergency_deep_cleanup ;;
        8) cleanup_housekeeping ;;
        9) emergency_purge_all ;;
        0) echo "Cancelled." ;;
        *) echo "Invalid choice." ;;
    esac
}
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - COMPLETE SERVER MANAGEMENT REFERENCE
# =================================================================
...
clear
echo -e "${WHITE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${WHITE}║          🆘 WSMS PRO v4.4.2 — COMMAND REFERENCE              ║${NC}"
echo -e "${WHITE}╚════════════════════════════════════════════════════════════╝${NC}"
...
# ============================================
echo -e "${BLUE}┌────────────────────────────────────────────────────────────┐${NC}"
echo -e "${BLUE}│  🔄 ROLLBACK SYSTEM — NEW in v4.4.2                           │${NC}"
echo -e "${BLUE}└────────────────────────────────────────────────────────────┘${NC}"
echo ""
...
printf "  ${GREEN}%-22s${NC} %s\n" "clamav-quarantine" "List quarantined files (/var/quarantine)"
...
# ============================================
echo -e "${BLUE}┌────────────────────────────────────────────────────────────┐${NC}"
echo -e "${BLUE}│  📝 LOG SHORTCUTS (/var/log/wsms/)                         │${NC}"
echo -e "${BLUE}└────────────────────────────────────────────────────────────┘${NC}"
echo ""
...
printf "  ${RED}%-30s${NC} %s\n" "Low disk space:" "backup-clean (use emergency options in menu)"
...
for site in "${SITES[@]}"; do
    ...
    printf "  ${GREEN}%-46s${NC} %s\n" "wp-update-site $name" "Update for $name"
    printf "  ${GREEN}%-46s${NC} %s\n" "mysql-backup $name" "Database backup for $name"
    echo ""
done
...
# ============================================
echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}✅ WSMS PRO v4.4.2 — READY FOR OPERATIONS${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
echo ""
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - INTERACTIVE BACKUP TOOL
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - RED-ROBIN SYSTEM BACKUP
# =================================================================
...
# Change to /tmp to prevent lock file clutter in $HOME
cd /tmp || exit 1
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - WP-CLI INFRASTRUCTURE VALIDATOR
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - ROLLBACK ENGINE
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - LOCAL HOSTS SYNC
# =================================================================
...
#!/bin/bash
# =================================================================
# WSMS PRO v4.4.2 - SYSTEM CLEANUP SCRIPT
...
echo -e "${CYAN}==========================================================${NC}"
echo -e "${CYAN}   🧹 WSMS PRO v4.4.2 - SYSTEM CLEANUP                       ${NC}"
echo -e "${CYAN}==========================================================${NC}"
...
alias clamav-clean-quarantine='sudo find /var/quarantine/ -mindepth 1 -delete && echo "✅ Quarantine cleaned"'
...
# ==================== FINAL SUMMARY ====================
echo -e "\n${GREEN}==========================================================${NC}"
echo -e "${GREEN}✅ WSMS PRO v4.4.2 INSTALLATION COMPLETE!${NC}"
echo -e "${GREEN}==========================================================${NC}"
echo ""
...
echo "   📝 Logs: /var/log/wsms/ (symlink: ~/logs/wsms/)"
