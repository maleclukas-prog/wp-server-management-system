...
# Autor: Lukasz Malec <github@lucasmalec.com> / GitHub: maleclukas-prog / Web: lucasmalec.com
# Licencja: MIT
...
uruchom_sprawdzian() {
    local etykieta="$1"
    local wynik
    wynik=$(bash "$SCRIPT_DIR/$2" 2>&1)
    RAPORT+="=== $etykieta ===\n$wynik\n\n"

    if echo "$wynik" | grep -qiE "❌|KRYTYCZNY|ERROR|failed|unreachable|stopped"; then
        ((BLEDY++))
    fi
}

uruchom_sprawdzian "Audyt zdrowia serwera"       "server-health-audit.sh"
uruchom_sprawdzian "Status floty WordPress"       "wp-fleet-status-monitor.sh"
uruchom_sprawdzian "Test infrastruktury WP-CLI"   "wp-cli-infrastructure-validator.sh"

if [ "$BLEDY" -gt 0 ]; then
    send_alert "failure" "Codzienny sprawdzian: $BLEDY problem(y) na $(hostname)" "$RAPORT"
else
    send_alert "success" "Codzienny sprawdzian: wszystkie systemy OK na $(hostname)" "$RAPORT"
fi
EOFDAILY
...
if ! systemctl is-active --quiet nginx && ! systemctl is-active --quiet apache2; then
    ...
    send_alert "failure" "KRYTYCZNY: Serwer WWW nie działa na $(hostname)" "Ani Nginx ani Apache2 nie jest uruchomiony.\nCzas: $(date)"
fi
...
uruchom_aktualizacje_strony() {
    ...
    send_alert "failure" "Aktualizacja nieudana: $name" "Strona: $name\nKod HTTP po aktualizacji: $http_code\nUruchomiono przywracanie.\nCzas: $(date)"
    ((fail_count++))
    return 1
}
...
if [ "$fail_count" -gt 0 ]; then
    send_alert "failure" "Cykl utrzymania zakończony z błędami" "Sukces: $success_count\nNiepowodzenie: $fail_count\nCzas: $(date)"
else
...
for site in "${SITES[@]}"; do
    IFS=':' read -r name path user <<< "$site"
    printf "  ${GREEN}%-46s${NC} %s\n" "wp-backup-site $name" "Kopia strony $name"
...
if [ -n "$target" ]; then
    ...
    if [ "$found" = true ]; then
        ...
        echo "❌ Nie znaleziono strony: $target"
...
if [ -n "$INFECTED" ]; then
    send_alert "failure" "ClamAV: znaleziono zainfekowane pliki na $(hostname)" "$(date)\n\n$INFECTED"
fi
EOFCLAM
